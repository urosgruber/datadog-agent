...
int __attribute__((always_inline)) discarded_by_inode(u64 event_type, u32 mount_id, u64 inode) {
    ...
    if (filter && (filter->event_mask & (1 << event_type))) {
#ifdef DEBUG
...
int __attribute__((always_inline)) discarded_by_pid(u64 event_type, u32 tgid) {
    ...
    if (params == NULL || (params->timestamps[event_type] != 0 && params->timestamps[event_type] <= bpf_ktime_get_ns())) {
        return 0;
    }
...
#endif
    return params->event_mask & (1 << event_type);
}
