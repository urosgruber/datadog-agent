...
type pidDiscarderParameters struct {
	EventType  EventType
	Timestamps [maxEventType]uint64
}
...
func (p *Probe) discardParentInode(rs *rules.RuleSet, eventType EventType, field eval.Field, filename string, mountID uint32, inode uint64, pathID uint32) (bool, error) {
	...
	err = p.discardInode(eventType, parentMountID, parentInode)
	return err == nil, err
}
